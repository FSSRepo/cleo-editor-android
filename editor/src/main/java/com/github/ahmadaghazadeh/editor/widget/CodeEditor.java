...
public class CodeEditor extends FrameLayout {
    ...
    public static void setCodeView(CodeEditor view, MutableLiveData<String> code, MutableLiveData<String> lang,
        ...
        if (lang != null) {
            view.language = new CLEOLanguage();
        }
    ...
    public OnTabClickListener tab_listener;
    ...
    private void init(Context context, AttributeSet attrs) {
        try {
            ...
            lines.add(0, 0);
            this.language = new CLEOLanguage();
            ...
            recyclerView.setListener((view, symbol) -> {
                        if (symbol.getShowText().endsWith("Tab") && tab_listener != null) {
                            String result = tab_listener.OnTabClick();
                            if(result != null) {
                                int start = editor.getSelectionStart();
                                editor.getText().insert(start, result);
                                String text = editor.getText().toString();
                                int s = text.indexOf("%", start);
                                if (s == -1) {
                                    editor.setSelection(text.length());
                                } else {
                                    editor.setSelection(s, text.indexOf("%", s + 1) + 1);
                                }
                            }
                        } else if (symbol.getShowText().endsWith("End")) {
                            String text = editor.getText().toString();
...
//    }
    ...
    @WorkerThread
    @Nullable
    public CLEOLanguage getLanguage() {
        return language;
    }
