...
public class TextProcessor extends AppCompatMultiAutoCompleteTextView implements View.OnKeyListener {
    ...
    private StylePaint mErrorPaint;
    ...
    protected void initTheme() {
        ...
        mErrorPaint = new StylePaint(false, false);

        if (!theme.resolveAttribute(R.attr.colorErrorLine, colorAttr, true)) {
            theme.resolveAttribute(R.attr.colorErrorLine, colorAttr, true);
            color = colorAttr.data;
            if (color == 0)
                color = getContext().getResources().getColor(R.color.colorErrorLine);
        }
        mErrorPaint.setColor(color);
        ...
        mSyntaxBrackets = new StyleSpan(color, false, true);
        ...
        mSyntaxMethods = new StyleSpan(color, true, false);
...
//                    return true;
    ...
    protected void setSuggestData() {
        SuggestionAdapter mAdapter =
                new SuggestionAdapter(mContext, R.layout.item_list_suggest);
        setAdapter(mAdapter);
    }
...
////                }
    ...
    public void onDraw(@NonNull Canvas canvas) {
        if (!isInEditMode()) {
            ...
            if (layout != null && codeEditor != null && mHighlightCurrentLine) {
                ...
                for(Integer line_error : errors) {
                    int selectedLineStartIndex = codeEditor.getIndexForStartOfLine(line_error);
                    int selectedLineEndIndex = codeEditor.getIndexForEndOfLine(line_error);
                    int topVisualLine = layout.getLineForOffset(selectedLineStartIndex);
                    int bottomVisualLine = layout.getLineForOffset(selectedLineEndIndex);
                    int left = mGutterWidth;
                    if (!mShowLineNumbers)
                    ...
                    int right = (layout.getWidth() + getPaddingLeft()) + getPaddingRight();
                    int bottom = layout.getLineBottom(bottomVisualLine) + getPaddingTop();
                    canvas.drawRect(left, top, right, bottom, mErrorPaint);
                }
            }
            super.onDraw(canvas);
    ...
    public void setCodeCompletion(boolean enabled) {
        ...
        if (!mAutoComplete) {
            ...
            setThreshold(1); //задержка перед отображением
        }
    }
